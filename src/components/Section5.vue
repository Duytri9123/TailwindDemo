<template>
  <div class="w-8/9 lg:w-6/8 mx-auto justify-center items-center  my-16 p-4">
    <div class="flex lg:flex-row flex-col justify-between gap-4">
      ...
      <div class="flex flex-col lg:items-center justify-center md:justify-end w-full">
        <button
          @click="toggleSearch"
          class="cursor-pointer bg-button hover:bg-button text-white font-semibold py-2 lg:px-8 px-4 rounded-full text-base flex lg:h-12 h-10 lg:max-w-72 max-w-44 items-center whitespace-nowrap justify-center gap-2"
        >
          <svg
            v-if="!showSearch"
            width="24"
        ...
        <div v-if="showSearch" class="w-1/3 mt-2">
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Tìm tên bác sĩ..."
            class="w-full p-2 border rounded-lg outline-none focus:ring focus:ring-green-300"
          />
        </div>
      </div>
    </div>
